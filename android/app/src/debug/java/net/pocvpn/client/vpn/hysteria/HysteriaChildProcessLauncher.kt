...
private const val TAG = "HysteriaChildProcess"

/** A live Hysteria child OS process - mirrors [Tun2SocksChildProcess]'s own shape (deliberately not reused directly - a different child, kept independently reviewable per B46-3C's own scope). */
interface HysteriaChildProcess {
    fun isAlive(): Boolean
    fun requestStop()
    fun forceStop()
...
interface HysteriaChildProcessLauncher {
    fun launch(binaryPath: String, args: List<String>): HysteriaChildProcess
}

/** Real launcher, mirrors `RealTun2SocksChildProcessLauncher`'s own shape - the only class that touches [java.lang.Process] directly for the Hysteria child. */
class RealHysteriaChildProcessLauncher : HysteriaChildProcessLauncher {
    override fun launch(binaryPath: String, args: List<String>): HysteriaChildProcess {
        val process = ProcessBuilder(listOf(binaryPath) + args)
            ...
            .start()
        return RealHysteriaChildProcess(process, binaryPath)
    }
}

private class RealHysteriaChildProcess(
    private val process: Process,
    private val binaryPathForLogging: String,
) : HysteriaChildProcess, LogLineObservable {
...
/** Filename the pinned minimal Hysteria2 ELF is packaged under in `src/debug/jniLibs/arm64-v8a/` (see docs/B46_3C_HYSTERIA_PROCESS_ISOLATED_DATAPLANE.md). */
const val HYSTERIA_CHILD_BINARY_FILENAME = "libnovahysteriachild.so"

/** Mirrors `Tun2SocksChildBinaryResolver`'s own "one place resolves the path" discipline. */
object HysteriaChildBinaryResolver {
    ...
    fun resolve(nativeLibraryDir: String?): Result {
        ...
        val file = java.io.File(nativeLibraryDir, HYSTERIA_CHILD_BINARY_FILENAME)
        return when {

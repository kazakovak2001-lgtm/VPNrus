...
private const val TAG = "Tun2SocksChildProcess"

/** A live child OS process - launcher-side handle only, never the authority on the child's own reported pid (see [Tun2SocksChildAck.Ok.pid]). */
interface Tun2SocksChildProcess {
    fun isAlive(): Boolean
...
interface Tun2SocksChildProcessLauncher {
    fun launch(binaryPath: String, args: List<String>): Tun2SocksChildProcess
}

/**
 * B46-3B - real [Tun2SocksChildProcessLauncher] backed by [ProcessBuilder].
 ...
 * reported over the wire in [Tun2SocksChildAck.Ok.pid].
 */
class RealTun2SocksChildProcessLauncher : Tun2SocksChildProcessLauncher {
    override fun launch(binaryPath: String, args: List<String>): Tun2SocksChildProcess {
        val process = ProcessBuilder(listOf(binaryPath) + args)
            ...
            .start()
        return RealTun2SocksChildProcess(process, binaryPath)
    }
}

private class RealTun2SocksChildProcess(
    private val process: Process,
    private val binaryPathForLogging: String,
) : Tun2SocksChildProcess {
...
/** Filename the pinned tun2socks-child ELF is packaged under in `src/debug/jniLibs/arm64-v8a/` (see docs/B46_3B_HYSTERIA_PROCESS_ISOLATION.md). */
 ...
 * packaging design.
 */
object Tun2SocksChildBinaryResolver {

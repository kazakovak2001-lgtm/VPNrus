...
class FileActivationReplayGuard(private val directory: File) : ActivationReplayGuard {
    ...
    override fun markRedeemed(envelope: ActivationEnvelope, nowEpochMillis: Long) {
        ...
        // File.renameTo does not reliably replace an existing destination on
        // every platform (POSIX/Android allows it; Windows does not) - the
        // destination is removed first so the rename below always lands on a
        // free path. This is not itself atomic, but the write above already
        // is (a reader either sees the old file or the new one, never a
        // partially-written one) - only the brief gap between delete and
        // rename is platform-dependent, and losing this local dedupe file to
        // a crash in that gap only removes a convenience refusal (see class
        // docs: server-side binding is the real replay bound).

...
private const val TAG = "Hysteria2Tun2SocksChildRuntime"
...
/** Result of a [Hysteria2Tun2SocksChildRuntime] lifecycle call - never a raw exception/crash. */
sealed interface Hysteria2Tun2SocksChildResult {
    data class Ok(val pid: Int) : Hysteria2Tun2SocksChildResult
    data class Failed(val reason: String) : Hysteria2Tun2SocksChildResult
}
 ...
 * [Hysteria2Tun2SocksChildRuntime]) specifically so [Hysteria2Tun2SocksChildRuntime] stays
 ...
 * NPE inside a real fd-close call in a JVM test - a [FakeHysteria2Hysteria2Tun2SocksDupFdCloser]
 ...
 */
fun interface Hysteria2Tun2SocksDupFdCloser {
    fun close(fd: Int)
}

/** Real fd closer - the only place `ParcelFileDescriptor.adoptFd` is called for the runtime's own pre-handoff fd cleanup. */
object RealHysteria2Hysteria2Tun2SocksDupFdCloser : Hysteria2Tun2SocksDupFdCloser {
 ...
 * close it themselves on any [Hysteria2Tun2SocksChildResult.Failed]:
 *  - On every validation/bind/launch failure BEFORE
 *    [Hysteria2Tun2SocksChildControlChannel.sendStartRequestAndAwaitAck] is ever
 *    called, this class closes `dupTunFd` itself via [dupFdCloser] before
 *    returning [Hysteria2Tun2SocksChildResult.Failed].
 *  - Once [Hysteria2Tun2SocksChildControlChannel.sendStartRequestAndAwaitAck] is
 *    called, IT owns closing `dupTunFd` from that point on (see
 *    [RealHysteria2Hysteria2Tun2SocksChildControlChannel]'s own `ParcelFileDescriptor
 ...
 * [start] registers [Hysteria2Tun2SocksChildProcess.onExit] so this class - not just
 ...
 * exit.
 */
class Hysteria2Tun2SocksChildRuntime(
    private val launcher: Hysteria2Hysteria2Tun2SocksChildProcessLauncher = RealHysteria2Hysteria2Hysteria2Tun2SocksChildProcessLauncher(),
    private val controlChannel: Hysteria2Tun2SocksChildControlChannel = RealHysteria2Hysteria2Tun2SocksChildControlChannel(),
    private val dupFdCloser: Hysteria2Tun2SocksDupFdCloser = RealHysteria2Hysteria2Tun2SocksDupFdCloser,
    ...
    private var process: Hysteria2Tun2SocksChildProcess? = null
    ...
    fun start(dupTunFd: Int, mtu: Int, socksAddr: String, binaryPath: String, controlSocketPath: File): Hysteria2Tun2SocksChildResult {
        synchronized(stateLock) {
            if (process != null) {
                ...
                return Hysteria2Tun2SocksChildResult.Failed("child already running (pid=$childPid)")
            }
        }
        if (dupTunFd < 0) {
            // Nothing to close - an already-invalid fd was never ours to own.
            return Hysteria2Tun2SocksChildResult.Failed("invalid fd")
        }
        if (mtu <= 0) {
            dupFdCloser.close(dupTunFd)
            return Hysteria2Tun2SocksChildResult.Failed("invalid mtu")
        }
        ...
        try {
            ...
            dupFdCloser.close(dupTunFd)
            return Hysteria2Tun2SocksChildResult.Failed("control channel bind failed: ${t.javaClass.simpleName}: ${t.message}")
        }
        ...
        val launched = try {
            ...
            dupFdCloser.close(dupTunFd)
            return Hysteria2Tun2SocksChildResult.Failed("failed to launch child process: ${t.javaClass.simpleName}: ${t.message}")
        }
        ...
        return when (ack) {
            is Hysteria2Tun2SocksChildAck.Ok -> {
                ...
                // RealHysteria2Hysteria2Tun2SocksChildProcess.onExit) fires the callback
                ...
                Hysteria2Tun2SocksChildResult.Ok(ack.pid)
            }
            is Hysteria2Tun2SocksChildAck.Failed -> {
                ...
                controlChannel.close()
                Hysteria2Tun2SocksChildResult.Failed(ack.reason)
            }
        }
    }
     ...
     */
    fun stop(): Hysteria2Tun2SocksChildResult {
        val current: Hysteria2Tun2SocksChildProcess?
        val stoppedPid: Int
        synchronized(stateLock) {
            current = process
            ...
            if (current == null) {
                ...
                controlChannel.close()
                return Hysteria2Tun2SocksChildResult.Ok(pid = -1)
            }
        ...
        controlChannel.close()
        return Hysteria2Tun2SocksChildResult.Ok(pid = stoppedPid)
    }

    /**
     * Runs on [RealHysteria2Hysteria2Tun2SocksChildProcess]'s own background watcher thread
    ...
    private fun stopLaunchedProcess(launched: Hysteria2Tun2SocksChildProcess) {

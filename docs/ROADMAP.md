...
| multi-provider gateway infrastructure | PLANNED | **B13 (2026-08-30) update**: a real second provider IS now deployed - Stockholm (AWS, eu-north-1) alongside Germany (Oracle Cloud), different provider AND different ASN, with AWG/REALITY/TLS_TCP all physically verified on both (see the Gateway Pool row above for the full evidence trail). `EndpointDescriptor.provider`/`.asn` (B11) are populated for both real endpoints, not just typed placeholders. Kept PLANNED, not IMPLEMENTED, because the capability this row names is the full multi-provider PRODUCT capability, and two pieces of it are still missing: (1) control-plane distribution - Stockholm has no `pocvpn-api` activation deployment, so its credentials were provisioned directly server-side rather than through the app's real self-service activation flow (see the Gateway Pool row's own "Deliberately NOT deployed for Stockholm" note); (2) automatic multi-gateway failover - selection between Germany and Stockholm today is manual only (`SelectedGatewayStore`/`SelectedProductionGatewaySource`), `SmartConnectCandidateSelector` picks a TRANSPORT within whichever gateway is already manually selected, it does not choose BETWEEN gateways (see the `automatic gateway failover` row below, still PLANNED, still blocked on this). |

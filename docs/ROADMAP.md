...
| VLESS+TLS/TCP handshake and tunneled traffic | **VERIFIED** (B8O2-ops, 2026-08-30) - real physical Android device (ADB-connected, Oppo CPH2173, debug build from merged `main`), manually forced to `UserTransportPreference.Manual(TLS_TCP)` (a debug-only diagnostics override - no product transport-selection UI exists yet, and no automatic failover was changed). `NovaXrayVpnService` logcat: `Xray core started`; `tun0` present at `172.19.0.1/30` (the Xray adapter's own plan, not AWG's). Server-side `nova-xray` journal shows real accepted connections tagged `[nova-vless-tls-in >> direct]` for this exact device's identity - authoritative proof of a genuine VLESS+TLS session, not merely "Connected" UI text. |

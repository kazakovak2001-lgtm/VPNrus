...
| TLS/TCP fallback | **FOUNDATION** | `XrayVlessTlsConfig`/`validateXrayVlessTlsConfig` (B8O1) are real and unit-tested: a materially simpler credential shape than REALITY (no `realityPublicKey`/`shortId`, just `serverName`/SNI against a real CA-issued cert). `XrayConfigRenderer` gained a `render(XrayVlessTlsConfig)` overload emitting `security: "tls"` + `tlsSettings` (`serverName`/`fingerprint`/explicit `allowInsecure: false`), reusing the SAME tun-inbound/vless-outbound shape REALITY already uses - confirmed against the pinned xray-core source in `docs/B8O0_TLS_TCP_FALLBACK_AUDIT.md`. REALITY's own rendering is unchanged (a structural non-regression test proves it). Kept at FOUNDATION, not IMPLEMENTED: `TransportKind.TLS_TCP` is NOT registered AVAILABLE anywhere, nothing constructs `NovaXrayVpnService` with this config, and no gateway-side TLS inbound/provisioning exists yet - config-and-renderer only, mirroring B8K1B's own "isolated adapter shell" precedent for REALITY. |
